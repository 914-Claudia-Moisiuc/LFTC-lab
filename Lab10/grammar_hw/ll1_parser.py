...
class LL1Parser:
    ...
    def parseSequence(self, sequence):
        ...
        while True:
            ...
            self.ds += ', ' + workingStack + ', ' + resultStack + ') -> '
            terminals = []
            if (len(inputStack) == 0 or ("epsilon" in inputStack and len(inputStack) == 1)) and workingStack == "":
                self.ds += 'accepted'
                self.printToFile("ps.txt", self.ds)
                return self.ds

            if isinstance(self.table[workingStack[0]][0], list):
                for elem in self.table[workingStack[0]]:
                    terminals.append(str(elem[0]))
            else:
                terminals.append(str(self.table[workingStack[0]][0]))
            ...
            if len(inputStack) == 0 and workingStack != "":
